...
async def login_password(payload: LoginPasswordRequest, db: AsyncSession = Depends(get_db)):
    ...
    if not spouse.face_enrolled:
        raise HTTPException(status_code=409, detail="Face not enrolled yet for this spouse")

    from jose import jwt
    expire = datetime.now(timezone.utc) + timedelta(minutes=5)
    challenge = jwt.encode(
        {"sub": str(spouse.id), "role": payload.role, "device_name": payload.device_name, "exp": expire, "type": "face_challenge"},
...
async def login_face(payload: LoginFaceRequest, db: AsyncSession = Depends(get_db)):
    ...
    return LoginFaceResponse(access_token=access_token, refresh_token=refresh_token, spouse_id=spouse.id, role=spouse.role.value)
...
@router.post("/password-reset/verify-face")
async def password_reset_verify_face(payload: PasswordResetVerifyRequest, db: AsyncSession = Depends(get_db)):
...
async def password_reset_complete(payload: PasswordResetCompleteRequest, db: AsyncSession = Depends(get_db)):
    ...
    if session.husband_verified_at is None or session.wife_verified_at is None:
        raise HTTPException(status_code=409, detail="Both spouses must pass face verification first")
