...
class VaultService {
  ...
  Future<Category> createCategory(Uint8List vmk, String scope, String name) async {
    final enc = await VaultCrypto.encryptText(vmk, name);
    final res = await _dio.post('/categories', data: {'scope': scope, 'enc_payload': enc});
  ...
  Future<List<Category>> listCategories(Uint8List vmk, String scope) async {
    final res = await _dio.get('/categories', queryParameters: {'scope': scope});
  ...
  Future<VaultEntry> createEntry(
    ...
    final res = await _dio.post('/vault/entries', data: {
  ...
  Future<List<VaultEntry>> listEntries(
    ...
    final res = await _dio.get('/vault/entries', queryParameters: {
      if (authorRole != null) 'author_role': authorRole,
      if (contentType != null) 'content_type': contentType,
      if (categoryId != null) 'category_id': categoryId,
      'favorites_only': favoritesOnly,
    });
    final entries = (res.data as List).map((e) => VaultEntry.fromJson(e)).toList();
  ...
  Future<ConsentRequestModel> requestEdit(Uint8List vmk, String entryId, String newText) async {
    final enc = await VaultCrypto.encryptText(vmk, newText);
    final res = await _dio.post('/vault/entries/$entryId/edit-request', data: {'enc_payload': enc});
  ...
  Future<List<ConsentRequestModel>> listConsentRequests({String status = 'pending'}) async {
    final res = await _dio.get('/vault/consent-requests', queryParameters: {'status': status});
    return (res.data as List).map((e) => ConsentRequestModel.fromJson(e)).toList();
  }

  Future<void> decideConsentRequest(String requestId, bool approve) async {
    await _dio.post('/vault/consent-requests/$requestId/decide', data: {'approve': approve});
  }
}
