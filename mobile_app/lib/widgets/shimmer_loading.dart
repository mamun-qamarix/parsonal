...
class AppShimmer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Shimmer.fromColors(
      ...
      highlightColor: isDark ? const Color(0xFF344338) : const Color(0xFFF6FAF7),
      child: child,
    );
  }
}
...
class _Block extends StatelessWidget {
  ...
  const _Block({this.width, required this.height, this.radius = const BorderRadius.all(Radius.circular(8))});

  @override
  Widget build(BuildContext context) {
    return Container(width: width, height: height, decoration: BoxDecoration(color: Colors.white, borderRadius: radius));
  }
}
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Row(
                children: [
                  const _Block(width: 32, height: 32, radius: BorderRadius.all(Radius.circular(16))),
                  const SizedBox(width: 10),
                  _Block(width: MediaQuery.of(context).size.width * 0.3, height: 12),
                ],
              ),
            ),
            const _Block(width: double.infinity, height: 220, radius: BorderRadius.zero),
            Padding(
              padding: const EdgeInsets.all(12),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _Block(width: MediaQuery.of(context).size.width * 0.6, height: 12),
                  const SizedBox(height: 8),
                  _Block(width: MediaQuery.of(context).size.width * 0.4, height: 12),
                ],
              ),
            ),
          ],
    ...
    return AppShimmer(
      child: ListView.builder(
        ...
        itemBuilder: (context, i) => Padding(
          ...
          child: Row(
            children: [
              const _Block(width: 40, height: 40, radius: BorderRadius.all(Radius.circular(20))),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _Block(width: MediaQuery.of(context).size.width * 0.5, height: 12),
                    const SizedBox(height: 6),
                    _Block(width: MediaQuery.of(context).size.width * 0.3, height: 10),
                  ],
                ),
              ),
            ],
